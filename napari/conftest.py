...
@pytest.fixture
def builtins(mock_npe2_pm: TestPluginManager):
    mf_path = str(Path(__file__).parent / 'builtins.yaml')
    with mock_npe2_pm.tmp_plugin(manifest=mf_path) as plugin:
        yield plugin


@pytest.fixture
def tmp_plugin(mock_npe2_pm: TestPluginManager):
    with mock_npe2_pm.tmp_plugin() as plugin:
        yield plugin
